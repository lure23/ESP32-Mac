...
Instructions on how to set up ESP32 toolchain using macOS and Multipass.

>If you rather work on Windows + WSL, see the very similar sister repo [ESP32 on WSL](https://github.com/lure23/ESP32-WSL).

<p />

>Note: Sandboxed development is definitely possible (and should be easy) by using virtualization software such as VirtualBox, Parallels and so on. We rule those out, because.. just because. The author is keen to know, whether Multipass can be used for embedded development.
...
- (a) Have [Multipass](http://multipass.run/) as the virtual sandbox, akin to what WSL is for Windows.

- (b) Have Windows connect to the device, with Multipass routing to it, via `usbip`.

   >Spoiler! This is the author's favourite approach. It means detaching the hardware completely from the desktop system, and requiring *no* driver installs on the developer account. That is, if you have a PC to spare..

- (c) Run IDE on Mac, but direct the Remote Development to the WSL instance on a Windows PC (where the development kit is physically connected).

   Did not get up to this, since the alternative 2 worked.
...
Mac:
- macOS 14.2.1
- CP210x device driver v. 6.0.2 (optional; alternative 1 only!)
- Multipass 1.13 RC
...
- Create an installation folder
   ...
   <!-- #contribute If you know a way how we can "clone" just the files, including "--recursive", please share the command. 
   -->
...
## Alternative B. Connecting the device through Windows

With Windows, we [know](https://github.com/lure23/ESP32-WSL) that we have a working `usbipd` server setup. If Multipass is able to see that IP, it should be able to use the hardware connected in the PC computer.
...
3. Bind the device
   ...
   ![](.images/win-usbipd-list.png)
   
   ```
   > usbipd bind 3-1
   ```

4. Multipass (under Mac): 
...
Notice the `Hello world!` (or whatever you changed it to) greeting, among the output.


>NOTE: Ctrl-C does not break you away from the `idf.py monitor`. The break key is `Ctrl-]` <sub>[source](https://espressif-docs.readthedocs-hosted.com/projects/esp-idf/en/v3.3.5/get-started/idf-monitor.html)</sub> but ON A Swedish/Finnish KEYBOARD that’s `Ctrl-å` /yeah!/.
>
>|locale|break key|
>|---|---|
>|en|`Ctrl-[`|
...
- [How to install usbip vhci_hcd drivers on an AWS EC2 Ubuntu Kernel Version](https://askubuntu.com/questions/1303403/how-to-install-usbip-vhci-hcd-drivers-on-an-aws-ec2-ubuntu-kernel-version) (Ask Ubuntu; Dec 2020) `[1]`
- [usbip error open vhci driver](https://unix.stackexchange.com/questions/470827/usbip-error-open-vhci-driver) (Unix & Linux; Sep 2018)
